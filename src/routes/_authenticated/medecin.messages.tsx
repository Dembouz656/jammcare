...
export const Route = createFileRoute("/_authenticated/medecin/messages")({ component: Page });
  ...
  return (
    <DashboardShell role={t("doctor")} name={name || t("doctor")} initials={initials} nav={nav}>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-soft">
        {msgs.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">{t("no_messages")}</p> : (
          <ul className="space-y-2">
            {msgs.map((m) => (
              <li key={m.id} className={`rounded-lg border px-4 py-3 ${m.read_at ? "border-border bg-surface" : "border-primary/40 bg-primary-soft"}`}>
                <div className="flex items-center justify-between gap-2"><p className="text-sm font-medium">{m.sender_name}</p><span className="text-xs text-muted-foreground">{new Date(m.created_at).toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" })}</span></div>
